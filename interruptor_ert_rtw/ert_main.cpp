...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(interruptor_M) == (NULL));
  while (runModel) {
    ...
    stopRequested = !((rtmGetErrorStatus(interruptor_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(interruptor_M, "stopping the model");
  runModel = 0;
}
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  rtmSetErrorStatus(interruptor_M, 0);
  ...
  // Initialize model
  interruptor_initialize();

  // External Mode initialization
  errorCode = extmodeInit(interruptor_M->extModeInfo, &rtmGetTFinal
    (interruptor_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(interruptor_M, true);
    }
  }
