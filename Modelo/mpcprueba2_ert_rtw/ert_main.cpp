...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(mpcprueba2_M) == (NULL));
  while (runModel) {
    ...
    stopRequested = !((rtmGetErrorStatus(mpcprueba2_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(mpcprueba2_M, "stopping the model");
  runModel = 0;
}
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  rtmSetErrorStatus(mpcprueba2_M, 0);
  ...
  // Initialize model
  mpcprueba2_initialize();

  // External Mode initialization
  errorCode = extmodeInit(mpcprueba2_M->extModeInfo, &rtmGetTFinal(mpcprueba2_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(mpcprueba2_M, true);
    }
  }
