...
extmodeSimulationTime_T getCurrentTaskTime(int_T tid)
{
  ...
  switch (tid) {
   case 0:
    extmodeTime = (extmodeSimulationTime_T)(mpc4_M->Timing.taskTime0);
    break;

   case 1:
    extmodeTime = (extmodeSimulationTime_T)(((mpc4_M->Timing.clockTick1) * 0.05));
    break;
  }
...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(mpc4_M) == (NULL));
  while (runModel) {
    ...
    stopRequested = !((rtmGetErrorStatus(mpc4_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(mpc4_M, "stopping the model");
  runModel = 0;
}
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  errorCode = extmodeInit(mpc4_M->extModeInfo, &rtmGetTFinal(mpc4_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(mpc4_M, true);
    }
  }
