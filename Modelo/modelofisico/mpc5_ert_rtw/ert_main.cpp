...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(mpc5_M) == (NULL));
  while (runModel) {
    ...
    stopRequested = !((rtmGetErrorStatus(mpc5_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(mpc5_M, "stopping the model");
  runModel = 0;
}
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  errorCode = extmodeInit(mpc5_M->extModeInfo, &rtmGetTFinal(mpc5_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(mpc5_M, true);
    }
  }
