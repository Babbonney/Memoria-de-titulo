...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(onlympc_M) == (NULL));
  while (runModel) {
    ...
    stopRequested = !((rtmGetErrorStatus(onlympc_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(onlympc_M, "stopping the model");
  runModel = 0;
}
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  errorCode = extmodeInit(onlympc_M->extModeInfo, &rtmGetTFinal(onlympc_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(onlympc_M, true);
    }
  }
