...
extmodeSimulationTime_T getCurrentTaskTime(int_T tid)
{
  ...
  switch (tid) {
   case 0:
    extmodeTime = (extmodeSimulationTime_T)(pmpc_M->Timing.taskTime0);
    break;

   case 1:
    extmodeTime = (extmodeSimulationTime_T)(((pmpc_M->Timing.clockTick1) * 0.05));
    break;
  }
...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(pmpc_M) == (NULL));
  while (runModel) {
    ...
    stopRequested = !((rtmGetErrorStatus(pmpc_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(pmpc_M, "stopping the model");
  runModel = 0;
}
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  errorCode = extmodeInit(pmpc_M->extModeInfo, &rtmGetTFinal(pmpc_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(pmpc_M, true);
    }
  }
