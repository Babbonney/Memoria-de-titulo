...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(mpcprueba3_M) == (NULL));
  while (runModel) {
    ...
    stopRequested = !((rtmGetErrorStatus(mpcprueba3_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(mpcprueba3_M, "stopping the model");
  runModel = 0;
}
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  rtmSetErrorStatus(mpcprueba3_M, 0);
  ...
  // Initialize model
  mpcprueba3_initialize();

  // External Mode initialization
  errorCode = extmodeInit(mpcprueba3_M->extModeInfo, &rtmGetTFinal(mpcprueba3_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(mpcprueba3_M, true);
    }
  }
