function K1 = k1setMPT(A,B,X,U,T,cap,lambda,E,W)
% K1SET Calculates the intersection between the robust one-step set
%          and the state constraints to form the robust one-step
%          controllable set.
%
%   Usage: 
%     K1 = k1set(A,B,X,U,T,cap,lambda,E,W)
%   
%   Inputs:
%     A,B,E:  State space matrices of the discrete-time 
%             LTI system x_{k+1} = A*x_k + B*u_k + E*w_k.
%             For polytopic uncertainty, stack vertices 
%             of uncertainty sets for A and B on top of each other.
%             The inclusion of E is optional.
%     X:      State constraints in polyhedron form (MPT toolbox).
%     U:      Control constraints in polyhedron form (MPT toolbox).
%     T:      Target set in polyhedron form (MPT toolbox).
%     cap:    Optional - Intersects one-step set with X.
%             1 if desired that the output be
%               K1 = {x \in X | \exists u \in U: Ax+Bu \in T} (default).
%             0 if desired that the output be
%               K1 = {x \in \Real^n | \exists u \in U: Ax+Bu \in T}.
%     lambda: Optional - default set to 1.
%             Contraction factor 0 < lambda <= 1.
%     W:      Optional - State disturbance set in polyhedron form (MPT toolbox).
%
%   Outputs: 
%     K1:   Robust one-step controllable set to lambda*T if cap=1.
%           Robust one-step set to lambda*T if cap = 0.
%           

% n: number of states
% m: number of inputs
[rA,n]=size(A);
[rB,m]=size(B);

% M: number of models
M = rA/n; % should also be equal to rB/n

if nargin >= 6
  if isempty(cap)
	cap = 1;
  end
else
  cap = 1;
end

if nargin >= 7
  if isempty(lambda)
    lambda = 1;
  end
else
  lambda = 1;
end

if lambda < 1
  T = lambda*T;
end

if nargin >= 9
  if ~isempty(W) 
    EW=W.affineMap(E);
    T = minus(T,EW);
    T.minHRep();
  end
end

if (rA==n) % If [A,B] certain then this method could be faster than
           % projection.
% if 0 % Replace previous if with this statement to disable this method 

  % Q = inv(A) * [T + (-B * U)]
  if m == n && isequal(B,eye(n))
    Q = T+U.affineMap(-eye(m));
    
%     QA = Q.affineMap(inv(A));
%     save('mpt','T','U','Q','QA')
    
    Q = Q.affineMap(inv(A));
    Q.minHRep();
  else
	Q = T+U.affineMap(-B);       % MOD 20/7/2001 - polymap(inv(A),...) was
                                 % applied here - not necessary - is
                                 % actually also done two lines later!
    Q = Q.affineMap(inv(A));
    Q.minHRep();
  end
  
  % Intersect with X
  if cap == 1
    Q = intersect(Q,X);
    Q.minHRep();
  end
  Q.minHRep();
  Q.minVRep();
  K1 = Q;
  
else % if (A,B) uncertain
  Xl=X.affineMap([eye(n);zeros(m,n)]);
  Ul=U.affineMap([zeros(n,m);eye(m)]);
  Xlf=Polyhedron(Xl.A,Xl.b);
  Ulf=Polyhedron(Ul.A,Ul.b);
  
  S=[T.A;T.Ae;-T.Ae];
  s=[T.b;T.be;-T.be];

  V = [];
  v = [];

  % Target set constraints for each model in A and B
  for j = 0:M-1
    V = [V; S*A(1+j*n:n+j*n,:) S*B(1+j*n:n+j*n,:)];
	v = [v; s];
  end
  Vp=Polyhedron(V,v);
  Vp.minHRep();

  % Add constraints on the control
  Vp=intersect(Vp,Ulf);
  Vp.minHRep();

  % Intersect with X
  if cap == 1
    Vp=intersect(Vp,Xlf);
    Vp.minHRep();
  end

  Vp.minHRep();
  Vp.minVRep();
  K1 = Vp.projection(1:n); % Projects onto the x subspace by solving
                           % the linear inequalities using the
                           % Fourier-Motzkin elimination method
  K1.minHRep();
  K1.minVRep();

end




